...
class FFmpegController: NSObject {
  ...
  public func generateThumbnailsForFile(atPath path: String) {
    ...
    currentWorkItem = DispatchWorkItem { [weak self] in
      ...
      `self`.delegate?.didGenerateThumbnails(`self`.thumbnails, forFileAtPath: path, succeeded: (`self` as AnyObject).synchronouslyGenerateThumbnailsForFile(atPath: path))
    }
...
extension FFmpegController {
  @objc func handleNewTimestamp(_ timestamp: Int64, progress: Int, forFileAtPath path: String) {
